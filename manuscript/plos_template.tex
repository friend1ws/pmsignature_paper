...
(see Supplementary Figure 1 for an example where the number of cancer genomes and mutations for each cancer genome is 25 and 100, respectively).
...
First, performed on various number of mutation signatures $K$ (including a background signature), we observed how the detected mutation signatures changes (See Supplementary Figure 3).
...
strong correlation between the AA signature (CpTpG $>$ CpApG) and that additional AA-like signature can be observed ($R = 0.77$, see Supplementary Figure 5). 
...
Considering all these factors together, $K = 3$ seems to be one reasonable choice in terms of the interpretability,
and we adopt $K = 3$ in the following.
...
On the other hand, for the full representation model, this tendency was rather mild (Figure \ref{UTUC:APOBEC_full5_sig}, \ref{UTUC:AA_full5_sig}).
...
For measuring the deviations of the mutation signature, the cosine similarity is used on the $\prod_{l=1}^L M_l$ dimensional vector space $f_{i, \bm{m}} = \prod_{l=1}^L f_{k,l,m_l}$,
so that the comparison between the full representation and the independent representation become possible.
Trials for downsampling experiment for each ratio and model were repeated for 100 times.

As the Figure \ref{UTUC:APOBEC_downsampling} and \ref{UTUC:AA_downsampling} shows, the independent representation could recover the original signatures even when about 90 \% of the data was removed.
...
The signature 13 (T $>$ [GT] at TpCpN sites) was observed in 11 cancer types, and related to the activity of APOBEC family.
...
For the signature 13, the signatures captured different cancer types are highly consistent (Figure \ref{APOBEC_sig_list}),
...
For the signature 7 and signature 1, 
...
In each division, 16 bars show joint probabilities of 16 patterns of immediate 5' and 3' bases combinations 
...
showing that this signature might have more compact representations.
(b) The strong intensities on TpCpT $>$ TpApT and TpCpG $>$ TpTpG are observed. This is a little complex in the sense that 
the immediate 3' base and the substitution pattern show strong correlation.
}
...
\caption{The summary of mutation signatures obtained in the reanalysis of the data of the previous study \cite{pmid23945592} using the proposed method,
...
\caption{The summary of membership of each mutation signature across 30 cancer types}
...
\subfigure[Signature intensities at two 5' to the mutated site]{%
  \includegraphics[width=7.5cm,height=7.5cm]{APOBEC_two5prime.eps}
  \label{APOBEC_twofive}}
  
\caption{APOBEC signatures obtained in each cancer type and the frequency of bases at two 5' to the mutated site}
...
\caption{Several signatures obtained in multiple cancer types}
