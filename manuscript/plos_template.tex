...
The first point is the assumption of independence, where we can identify mutation signatures as multiple multinomial distribution parameters on multiple contextual factors.
...
This representation will help us to immediately identify which elements are strongly working on each contextual factors.
...
The second novel point is that we adopt  the ``mixed membership model'' for the mutation signature problem.
Briefly, the mixed membership model is basically clustering method, but allowing that each individual can have multiple clusters.
The mixed membership model is widely used in many different fields such as population structure classification in population genetics and document classification problems in machine learning.
This model is also shown to be closely related to nonnegative matrix factorization.


Suppose there are in total $K$ source of mutations (or mutation signatures) in the cohort of interest.
...
Here, by convention of the mixed membership model, we assume that each mutation for each cancer genome is generated by a two step model:
First, one of the contributing mutation signature is chosen depending on the membership parameter of that cancer genome.
...
To account for the intrinsic sequential context, we add background signatures.
See the Method section for the mathematical details, 
detailed relationships with mixed membership models and nonnegative matrix factorizations, how to choose the number of mutation signatures and so on.
...
The proposed method uses two signatures for representing the signature for the Pol $\epsilon$ dysfunction, 
...
Although the replication strand bias is different from the transcription strand bias, these two biases may have strong correlation
through the fact that replication origins prefer transcription start sites \cite{pmid23187890}.
Therefore, the transcription strand bias observed in the signature 1 may actually come from the replication strand biases.

Furthermore, both the signature 1 and 8 signatures shows strong abundance of the T base at the $-2$ position,
and the signature 1 also shows a little abundance of the T base at the $+2$ position.
 (Figures \ref{POLE1_sig_list} and \ref{POLE2_sig_list}, Supplementary Figure 7(b, c)).
In the previous study of Pol $\epsilon$  \cite{pmid25228659}, 
a nonsense mutation in TP53 at R23X were shown to be enriched in cancers with Pol $\epsilon$ defects.
In fact, the pattern of this mutation is C $>$ T at TpTpCpGpA, that is the most likely patten with the signature 8.
...
meaning that significant ratios of somatic mutations corresponding to that signature could not be validated by re-sequencing or visual inspection of BAM files using genomic viewers.
Actually, the pattern of this signature largely matches to that of the putative artifact suggested in \cite{pmid23303777}, 
that is caused by oxidation of DNA during acoustic shearings, 
and we guess that this signature is the same as what is observed in that study.
Although this signature may be from the artifacts, 
knowing higher resolution of the artifact signature is very helpful to accurately remove false positive mutations.
...
The composition of nucleotides of estimated signatures in 12 different cancer types are highly consistent (Figure \ref{APOBEC_sig_list}) except for the on in B-cell lymphoma,
...
Actually, this signature does not match to any of signatures in the previous paper \cite{pmid23318258}.
...
Even though the proposed method impose an independence assumptions among each contextual factor,
the proposed method could capture many of the previously detected mutation signatures.
Also, we have shown that the proposed method with the independence assumption is gives more robust estimates compared to that without the independence assumption.
...
As we have discussed, the independence assumption may not be completely consistent with the nature of DNA damages and repairs.
Still, we have demonstrated that the model with the independence assumption is ``practically'' very helpful.
...
such as GC contents, repeat sequences, chromatin accessibility and modifications, and replication timing and so on \cite{pmid22820252, pmid21953857, pmid23422670, pmid23770567}.
Also, a recent study has shown that the epigenetic information of the cell type of origin of the corresponding tumors is the most predictive \cite{pmid25693567}.
As a number of epigenetic data from various type of cells accumulate,
one of the next main topic will be to integrating those epigenetic factors into the mutation signature 
