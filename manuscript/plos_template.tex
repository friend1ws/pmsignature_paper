...
The signature 13 (T $>$ [AGT] at TpCpN sites) was observed in 12 cancer types, and related to the activity of APOBEC family.
...
In terms of transcribed strand-specificity, the tendency is not consistent, implying that there is no significant strand-specificity in APOBEC signatures.


The signature 1 and 8 (C $>$ A at TpCpT and C $>$ T at TpCpG, respectively) were observed in colorectal and uterine cancers,
are probably associated with deregulated activity of the error-prone polymerase Pol $\epsilon$.
...
Although replication strand biases is different from the transcription strand biases, these two biases may have strong corelation,
...
Furthermore, both the signature 1 and 8 signatures showed strong abundance of T base at $-2$ positions,
...
The signature 18 (T $>$ C at ApTp[AG]), observed in liver cancer, shows very strong strains specificity,
were shown to be more common in Asian cases than in other ancestries \cite{pmid25362482}, though the source of this signature is still not clear.


The signature 3 and 5 (C $>$ A at NpCpN), which were observed in head-and-neck cancers, three types of lung cancers,
are probably associated with tobacco smoking.
Furthermore, the fact that the estimated signature for each cancer shows higher prevalence on coding strands is consistent with the previous study.
...
and seems to be associated with a chemotherapy drug, temozolomide.
...
The signature 2 (C $>$ A at [CT]pCpT), observed in low grade gliomas, can be related to the one detected in the same cancer type in the previous study, but slightly different. 
...
Actually, the mutation related to this signature is mostly coming from one cancer with a very high mutation rate.
When we re-analyze the data removing this sample, the signature 2 disappeared.
We suspect that the low-grade-glioma specific signature detected in the previous study may be aggregation of multiple signatures,
but cannot be decomposed due to the lack of samples having these signatures.
Therefore, we need to be more careful for the signature 2 and the corresponding signature in the previous study.

The signature 3 (C $>$ A at CpCpG), observed in several cancers,
Actually, this signature correspond to the one that could not be validated in the previous study,
and the pattern 
...
Although, our method detected most of the 
