...
for (i in newOrder) {
  ...
  visPMS_ind(Fs[[i]], numBases = 5, trDir = TRUE, charSize = 2.1, charLimit = 0.333);
...
outputName <- "../presentation/AlexandrovEtAl_mergedSignature.eps";
...
outputName <- "../presentation/corrplot.eps"
...
for (i in 1:length(importantSigs)) {
  ...
  for (j in 1:length(types)) {
    ...
    if (length(types) > 4) {
      visPMS_ind(F, numBases = 5, trDir = TRUE, charSize = 2, charLimit = 0.333);
    } else {
      visPMS_ind(F, numBases = 5, trDir = TRUE, charSize = 0.6 + 0.4 * length(types), charLimit = 0.333);
    }      
  ...
  outputName <- paste("../presentation/signatureList_", names(importantSigs)[i], ".eps", sep="");
...
names(importantSigs2) <- c("LUNG_merge", "UV_merge", "TCR");
...
for (i in 1:length(importantSigs2)) {
  ...
  outputName <- paste("../presentation/signatureList_", names(importantSigs2)[i], "_nonBase.eps", sep="");
  ...
  # the setting of charSize is temporary... why the size changes by the number of rows.....
  visPMS_ind(F, numBases = 5, trDir = TRUE, charSize = 1.4, charLimit = 0.333);
  outputName <- paste("../presentation/signatureList_", names(importantSigs2)[i], ".eps", sep="");
  dev.copy2eps(file=outputName, height = 2.4, width = 3.75, pointsize = 1e-10);
}
