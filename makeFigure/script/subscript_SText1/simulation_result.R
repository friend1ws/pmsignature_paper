...
for (i1 in 1:7) {
  for (i2 in 1:4) {
    for (i3 in 1:3) {
      for (i4 in 1:3) {
        a <- read.table(paste("../../analysis/simulation/result/cosineDist/", mutNum[i1], "_", sampleNum[i2], "_", alpha[i3], "_", gamma[i4], ".txt", sep=""), sep="\t", header=FALSE);
...
ggsave("../result/SText1/simulation_result.eps", width=15, height=6);
