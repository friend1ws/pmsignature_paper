...
while(<IN>) {
    ...
    # print $sample2index{$F[1]};
    print $F[1];

    my $mutFeatID = 0;
    if ($numBase == 3) {
        $mutFeatID = ($base2ind{substr($fSeq, 3, 1)} + 4 * ($base2ind{substr($fSeq, 1, 1)} - 1) + 16 * ($mut2ind{substr($fSeq, 2, 1) . ">" . $altBase} - 1));
    } elsif ($numBase == 5) {
        $mutFeatID = ($base2ind{substr($fSeq, 4, 1)} + 4 * ($base2ind{substr($fSeq, 0, 1)} - 1) + 16 * ($base2ind{substr($fSeq, 3, 1)} - 1) + 64 * ($base2ind{substr($fSeq, 1, 1)} - 1) + 256 * ($mut2ind{substr($fSeq, 2, 1) . ">" . $altBase} - 1));
    }
    ...
    if ($dirOrNot == 0) {
        ...
        if ($numBase == 3) {
            $mutFeatID = $mutFeatID + 192 * ($fStrand - 1);
        } elsif ($numBase == 5) {
            $mutFeatID = $mutFeatID + 1536 * ($fStrand - 1);
        }
        print "\t" . $mutFeatID . "\n";
    }
