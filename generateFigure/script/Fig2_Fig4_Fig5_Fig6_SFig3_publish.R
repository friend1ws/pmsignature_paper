...
for (i in newOrder) {
  ...
  visPMS_ind(Fs[[i]], numBases = 5, trDir = TRUE, charSize = 0.6);
  ...
  mtext(paste("signature", curSig),
        ...
        line = -0.8,          # draw at 0.5 line toward outside 
        col = "black")    
  curSig <- curSig + 1;
...
# dev.copy2eps(file=outputName, height = ceiling(length(Fs) / 4) * 1.2, width = 7.5, pointsize = 18);
outputName <- "../result/Fig4/AlexandrovEtAl_mergedSignature.tiff";
cop <-dev.copy(tiff,filename=outputName,width=7.5,height=ceiling(length(Fs) / 4) * 1.2,units="in",pointsize = 18,res=600,compression="lzw")
dev.flush()
dev.off()
...
corrplot(sig2type, is.corr=FALSE, bg="#F8F8FF", col=myCol(200), tl.col="black", tl.cex=0.6, tl.srt=90, cl.pos="n");

# outputName <- "../result/Fig5/corrplot.eps"
# dev.copy2eps(file=outputName, height = 7.5, width = 7.5, pointsize = 12);
outputName <- "../result/Fig5/corrplot.tiff"
cop <-dev.copy(tiff,filename=outputName,width=7.5,height=7.5,units="in",pointsize = 12,res=600,compression="lzw")
dev.flush()
dev.off()
