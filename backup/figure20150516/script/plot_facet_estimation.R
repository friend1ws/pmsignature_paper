...
for (i1 in 1:7) {
  for (i2 in 1:4) {
    for (i3 in 1:3) {
      for (i4 in 1:3) {
        a <- read.table(paste("../result/cosineDist/", mutNum[i1], "_", sampleNum[i2], "_", alpha[i3], "_", gamma[i4], ".txt", sep=""), sep="\t", header=FALSE);
...
ggplot(summary_simulation.result, aes(x=factor(mutNum), y=cosine_similality, ymax=1, ymin = 0.3, fill=factor(sampleNum), colour=factor(sampleNum), group=factor(sampleNum))) +
  ...
  geom_point(position=pd, size=2, shape=21, alpha=0.5) +
...
ggsave("facet_result.eps", width=12, height=5);
