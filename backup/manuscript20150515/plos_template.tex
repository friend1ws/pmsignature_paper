...
\begin{flushleft}
{\Large
\textbf{A simple model-based approach to inferring and visualizing cancer mutation signatures}
}
...
\\
$\ast$ E-mail: yshira@hgc.jp; mstephens@uchicago.edu
\end{flushleft}
...
greatly simplify mutation signature models compared with existing approaches, reducing the number of parameters by orders of magnitude even while increasing the contextual factors (e.g.~number of flanking bases) that are accounted for. This improves both sensitivity and robustness of inferred signatures. We also provide a new intuitive way to visualize the signatures, analogous to the use of sequence logos to visualize transcription factor binding sites. 
...
Figure \ref{mutSig_example} illustrates the way that our new representation of signatures can simply capture
...
$\bm{M} = (6, 4, 4, 4, 4)$.
See Supplementary Table 1 for other examples.
...
This generative model is summarized in Figure \ref{methodOverview}.
...
provided enough mutations and samples are available (see Supplementary Material and Supplementary Figure 1). 
...
we selected $K=3$ signatures for these analyses (Supplementary Material, Supplementary Figure 2, 3 and 4).


The inferred APOBEC signature under the independent model
shows a clear depletion of G base at the $-2$ position,
which is consistent to the previous study \cite{pmid23318258} and results in the next subsection.
In contrast, for the full model, this tendency is rather mild (Figure \ref{UCUT:APOBEC_full5_sig}, \ref{UCUT:AA_full5_sig} and Supplementary Figure 5).
...
The results (Figures \ref{UCUT:APOBEC_downsampling} and \ref{UCUT:AA_downsampling})
...
For each cancer type we selected the number of signatures $K$ by fitting the model with increasing $K$ and examining the log-likelihood, bootstrap errors, and correlation of membership parameters.The selected values of $K$ are given in 
...
Figures \ref{nature2013_sig_summary} and \ref{nature2013_sig_member} summarise the results.
...
signature 1 shows a transcription strand bias whereas signature 8 does not, and this is true for both colorectal and uterus cancers (Supplementary Figures 6(c, d)).
...
and signature 1 also shows slightly elevated frequency of the T base at position $+2$
 (Figures \ref{POLE1_two5prime} and \ref{POLE2_two5prime}, Supplementary Figure 6(c, d)).
...
appears related to, but slightly different from, the signature previously detected in the same cancer types (Supplementary Figure 7 (a)). Indeed, the corresponding signature in the previous study shows very complex patterns (C $>$ A at NpCpT or C $>$ T at GpTpN). Further investigation
revealed that this signature is driven by 
a single sample with an extremely high mutation rate (Supplementary Figure 7 (b,c)), and signature 2 disappeared when we removed this sample (Supplementary Figure 8).
...
Again our analysis highlights additional contextual information, with a strikingly elevated frequency of base C at the $-2$ position (Supplementary Figure 9 (a,b,c)).
However, for each cancer type, only a few samples support this signature (see Supplementary Figure 9 (d, e, f)),
...
Signature 13 (T $>$ [AGT] at TpCpN sites) was observed in 12 cancer types, and is surely related to the activity of the APOBEC family. The 12 inferred signatures are highly consistent among cancer types except for B-cell lymphoma (Supplementary Figure 6(a)), highlighting the robustness
of our approach.  Almost all of them show enrichment of A and T and depletion of G base at the $-2$ position (Figure \ref{APOBEC_two5prime}, Supplementary Figure 6(a)), 
...
Consistent strand specificities among the two cancer types (Supplementary Figure 6(e)) matches previous results \cite{pmid23318258}, but our analysis additionally highlights elevated abundance of T at the $-2$ position (Figure \ref{UV_two5prime}, Supplementary Figure 6(e)). 
Signature 11 (C $>$ T at GpCp[CG]) appears in small-cell lung cancers and stomach cancers,
and seems to be the same as ``signature 15'' in the previous study, whose function remains unclear. Again our analysis highlights elevated abundance of G at the $-2$ position (Figure \ref{LMST_two5prime}, Supplementary Figure 6(f)).
...
The estimated signature in each cancer type shows higher mutation prevalence on the template strand (Supplementary Figure 6(b)), 
...
See Supplementary Material for the derivation of the above updating procedures.
...
existing approaches have one feature with $\bm{M} = (M_1 \times \dots \times M_L)$ (see Supplementary Table 1). 
...
See Supplementary Materials for details of the relationship between the proposed approach and nonnegative matrix factorization.
...
% there are many rooms for improvement in the current form of the proposed method by learning from past experiences and knowledge.
...
\subfigure[Example mutation signature representation for the unconstrained model]{%
   \includegraphics[width=15cm,height=4cm]{example3076_TCR.eps}
  \label{TCR_ful}
}

\subfigure[Example mutation signature representation for the independent model]{%
   \includegraphics[width=8cm,height=5.32cm]{example18_TCR.eps}
  \label{TCR_ind}
}
\quad
\subfigure[Mutation signature parameter values for the signature in (b)]{%
    \includegraphics[width=175pt]{exampleTable.pdf}
    \label{TCR_ind_table}
}

\caption{Examples of visualizations and parameter values for the mutation signatures of the unconstrained (full) model and our independent model, 
...
\caption{An overview of the generative model of somatic mutations proposed in this paper. Suppose there are three types of mutation sources (mutation signatures) such as ultraviolet, tobacco smoking chemicals and transcription coupled repairs.
...
\subfigure[APOBEC signature for the independent model]{%
  \includegraphics[height=5cm,width=7.5cm,clip]{UCUT_APOBEC_ind.eps}
  \label{UCUT:APOBEC_ind5_sig}}
\quad
\subfigure[AA signature for the independent model]{%
  \includegraphics[height=5cm,width=7.5cm,clip]{UCUT_AA_ind.eps}
  \label{UCUT:AA_ind5_sig}}
  
  \subfigure[APOBEC signature for the full model]{%
  \includegraphics[height=5cm,width=7.5cm,clip]{UCUT_APOBEC_full.eps}
  \label{UCUT:APOBEC_full5_sig}}
\quad
\subfigure[AA signature for the full model]{%
  \includegraphics[height=5cm,width=7.5cm,clip]{UCUT_AA_full.eps}
  \label{UCUT:AA_full5_sig}}
  
\subfigure[APOBEC signature stability]{%
  \includegraphics[height=5.5cm,width=7.5cm,clip]{UCUT_downsampling_APOBEC.eps}
  \label{UCUT:APOBEC_downsampling}}
\quad
\subfigure[AA signature stability]{%
  \includegraphics[height=5.5cm,width=7.5cm,clip]{UCUT_downsampling_AA.eps}
  \label{UCUT:AA_downsampling}}
%
\caption{The mutation signatures for the UCUT data, and the results of downsampling experiments. 
